...
impl LogIndex {
    ...
    pub fn load(dir: &Path) -> io::Result<Option<Self>> {
        ...
        loop {
            ...
            match tag[0] {
                ...
                TAG_FILE_REMOVED => match read_u32(&mut r) {
                    Ok(path_id) => {
    ...
    pub fn find_files(&self, word: &str) -> Vec<PathBuf> {
        ...
        match self.postings.get(&wh) {
            Some(file_ids) => file_ids
                .iter()
                .filter_map(|&fid| self.path_table.get(fid as usize).map(|s| PathBuf::from(s)))
                .collect(),
            None => Vec::new(),
        }
    }
    ...
    pub fn memory_usage(&self) -> usize {
        ...
        let postings: usize = self
            ...
            .iter()
            .map(|(_, v)| {
                std::mem::size_of::<u32>() + v.len() * 4 + std::mem::size_of::<Vec<u32>>()
            })
            .sum();
        // Per-file word_hashes kept for incremental updates
        let file_hashes: usize = self.files.values().map(|fd| fd.word_hashes.len() * 4).sum();
        let file_symbols: usize = self
            .files
            .values()
...
impl LogWriter {
    ...
    pub fn write_file_data(
        ...
        self.w
            .write_all(&(word_hashes.len() as u32).to_le_bytes())?;
...
mod tests {
    ...
    fn write_and_load_log() {
...

        {
            ...
            w.write_file_data(pid_lib, 1001, &hashes_lib, &[], Some(LangFamily::Rust))
                .unwrap();
    ...
    fn incremental_append() {
...

        {
            ...
            w.write_file_data(pid, 100, &[word_hash("alpha")], &[], None)
                .unwrap();
            w.flush().unwrap();
        }
    ...
    fn file_removal() {
...

        {
            let mut w = LogWriter::create(dir.path()).unwrap();
            let pid = w.intern_path(Path::new("/a.rs")).unwrap();
            w.write_file_data(pid, 100, &[word_hash("x")], &[], None)
                .unwrap();
            w.flush().unwrap();
        }
