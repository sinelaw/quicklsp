...
; ── Local assignments (inside any function scope) ───────────────────────
; Simple `x = …`
(assignment
  left: (identifier) @name) @definition.local

; Tuple / list destructuring `x, y = …` and `[x, y] = …`
(assignment
  left: (pattern_list (identifier) @name)) @definition.local

(assignment
  left: (tuple_pattern (identifier) @name)) @definition.local

(assignment
  left: (list_pattern (identifier) @name)) @definition.local
...
/// Python nodes that introduce a new function scope.
const PYTHON_SCOPE_KINDS: &[&str] = &["function_definition", "lambda"];
...
// and `@definition.parameter` rules in `PYTHON_QUERY` above. The shared
// query engine walks up to `function_definition`/`lambda` ancestors to
// compute depth / scope_end_line automatically.
