...
"#;
        ...
        assert!(names.contains(&"hello"), "should find method hello, got: {:?}", names);
        assert!(names.contains(&"_helper"), "should find method _helper, got: {:?}", names);
        ...
        let get_name = result.symbols.iter().find(|s| s.name == "get_name").unwrap();
