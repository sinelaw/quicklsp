...
; The engine only promotes these to locals when the match is inside a
; function scope; top-level declarations stay as globals (handled above).
(variable_declarator
...
fn parse_ts_common(source: &str, language: tree_sitter::Language) -> ParseResult {
    common::run_query_parse(
        ...
        &QueryParseConfig {
            ...
            scope_kinds: super::javascript::JS_SCOPE_KINDS,
...
/// and `@definition.parameter` SCM rules in `TS_QUERY`.
