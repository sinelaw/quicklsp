...
fn main() {
    ...
    println!("workspace.find_definitions({name}): {} results", ws_defs.len());
    for d in &ws_defs {
        println!("  {} line {} in {}", d.symbol.def_keyword, d.symbol.line, d.file.display());
    }
    
    if ws_defs.is_empty() {
        let dep_defs = dep.find_definitions(name);
        println!("dep_index.find_definitions({name}): {} results", dep_defs.len());
        for d in &dep_defs {
            println!("  {} line {} in {}", d.symbol.def_keyword, d.symbol.line, d.file.display());
        }
    }
    ...
    let name = "Client";
    let ws_defs = ws.find_definitions(name);
    println!("\nworkspace.find_definitions({name}): {} results", ws_defs.len());
    if ws_defs.is_empty() {
        let dep_defs = dep.find_definitions(name);
        println!("dep_index.find_definitions({name}): {} results", dep_defs.len());
    }
